...
for outer_sample_index = 1:number_of_samples
  ...
  # run the regular model to start time -
  (T,X) = add_atra_simulation(time_start,time_stop,time_step_size,copy_data_dictionary);
  initial_condition_array = X[end,:]

  number_of_parameters = length(parameter_mapping_array)
  for parameter_index = 1:number_of_parameters

    # reset -
    copy_data_dictionary["initial_condition_array"] = initial_condition_array

    message_string = "Starting $(outer_sample_index) of $(number_of_samples) for parameter $(parameter_index) of $(number_of_parameters)"
    println(message_string)

    # run the adj simulation -
    (time_array,simulation_state_array) = add_atra_adj_simulation(T[end],T[end]+1.0,time_step_size,parameter_index,copy_data_dictionary);

    # dump data to disk -
    local_data = [time_array simulation_state_array];
    data_filename = "./adj_simulation/sim_data_E1_EI_"*string(sample_index)*"_LI_"*string(outer_sample_index)*"_P"*string(parameter_index)*".dat"
